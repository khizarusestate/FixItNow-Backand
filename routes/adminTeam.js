...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const sanitized = admins.map((admin) =>
      buildAdminTeamMember(admin, isAdminConnected(String(admin._id))),
    );

    const stats = buildAdminTeamStats(sanitized);
    return res.json({
      success: true,
      data: sanitized,
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    logAudit(req, AUDIT_ACTIONS.ADMIN_CREATE, newAdmin._id, {
    ...
    return res.status(201).json({
      ...
      data: buildAdminTeamMember(newAdmin, false),
    });
  }),
);
...
router.patch(
  ...
  asyncHandler(async (req, res) => {
    ...
    logAudit(req, AUDIT_ACTIONS.ADMIN_UPDATE, target._id, {
      email: target.email,
    ...
    return res.json({
      ...
      data: buildAdminTeamMember(
        refreshed,
        isAdminConnected(String(refreshed._id)),
      ),
    });
  }),
);
...
router.patch(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (target._id.toString() === req.admin.id && !isActive) {
      return res.status(400).json({
        success: false,
        message: "You cannot set your own account to Inactive.",
      });
    }
    ...
    if (!isActive) {
      await forceLogoutAdmin(
        target._id,
        "Your admin account was set to Inactive by the super admin.",
      );
    }
    ...
    return res.json({
      ...
      message: isActive
        ? "Admin is now Active."
        : "Admin is now Inactive.",
      data: buildAdminTeamMember(target, isAdminConnected(String(target._id))),
    });
  }),
);
