...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    await Booking.findOneAndUpdate(
      { _id: bookingId, isDeleted: false, status: { $in: OPEN_STATUSES } },
      {
        ...
        $push: {
          timeline: {
            status: BOOKING_STATUS.CLAIM_PENDING,
            timestamp: new Date(),
            note: `Claim submitted by ${worker.fullName}. Commission ₨${commissionAmount}. Txn: ${transactionId}`,
          },
        },
      },
    );
    ...
    return res.json({
      success: true,
      message:
        "Claim submitted. Admin will verify your commission payment shortly.",
      data: {
