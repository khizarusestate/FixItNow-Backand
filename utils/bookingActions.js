...
const ADMIN_TRANSITIONS = {
  ...
  approved: ["in-progress", "rejected", "cancelled"],
...
function getCustomerCompleteBlock(booking, serviceTitle) {
  ...
  const blocks = {
    pending: blockPayload({
      ...
      message: `${serviceTitle} is still pending admin approval. You can mark it done only after a worker is assigned.`,
      status: 400,
    }),
    rejected: blockPayload({
      code: ERROR_CODES.BOOKING_ALREADY_REJECTED,
...
function getWorkerClaimBlock(status, serviceTitle, context) {
  ...
  const blocks = {
    pending: blockPayload({
      ...
      message: `${serviceTitle} is still waiting for admin approval and cannot be claimed yet.`,
      status: 400,
    }),
    rejected: blockPayload({
      code: ERROR_CODES.BOOKING_NOT_AVAILABLE,
