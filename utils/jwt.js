...
export const createAccessToken = (payload) => {
  ...
  const tokenPayload = { id: payload.id, role: payload.role, email: payload.email || null, iat: Math.floor(Date.now() / 1000) };
  return jwt.sign(tokenPayload, JWT_SECRET, { expiresIn: `${env.ACCESS_TOKEN_EXPIRY_MINUTES}m`, jwtid: crypto.randomUUID(), algorithm: "HS256" });
};
...
export const createToken = (payload) => {
  ...
  const tokenPayload = { id: payload.id, role: payload.role, email: payload.email || null, iat: Math.floor(Date.now() / 1000) };
  return jwt.sign(tokenPayload, JWT_SECRET, { expiresIn: `${JWT_CONFIG.LEGACY_TOKEN_DAYS}d`, jwtid: crypto.randomUUID(), algorithm: "HS256" });
};
...
export const createRefreshToken = async (userId, userRole, req = null, expiryDays = null) => {
  ...
  await RefreshToken.create({ token, userId, userRole, expiresAt, ipAddress: req?.ip || "", userAgent: req?.headers["user-agent"] || "" });
  return token;
};
...
export const revokeRefreshToken = async (token) => {
  ...
  const result = await RefreshToken.updateOne({ token }, { isRevoked: true, revokedAt: new Date() });
...
export const revokeAllUserRefreshTokens = async (userId, userRole) => {
  ...
  const result = await RefreshToken.updateMany({ userId, userRole, isRevoked: false }, { isRevoked: true, revokedAt: new Date() });
...
export const verifyToken = (token) => {
  ...
  try {
    ...
    if (error.name === "TokenExpiredError") { const normalized = new Error("Token expired"); normalized.name = "TokenExpiredError"; throw normalized; }
    if (error.name === "JsonWebTokenError") { const normalized = new Error("Invalid token"); normalized.name = "JsonWebTokenError"; throw normalized; }
    if (error.name === "NotBeforeError") { const normalized = new Error("Token not yet valid"); normalized.name = "NotBeforeError"; throw normalized; }
    throw error;
  }
};
...
export const decodeToken = (token) => {
  ...
  try { return jwt.decode(token); } catch (error) { logger.warn("Failed to decode token", { error: error.message }); return null; }
};

export const isTokenExpired = (token) => {
  if (!token || typeof token !== "string") return true;
  try { const decoded = jwt.decode(token); if (!decoded || !decoded.exp) return true; return decoded.exp < Math.floor(Date.now() / 1000); } catch { return true; }
};
